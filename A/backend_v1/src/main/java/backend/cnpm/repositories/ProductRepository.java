...
import backend.cnpm.dto.response.UserProductResponse;
import backend.cnpm.entities.Product;
...
public interface ProductRepository extends JpaRepository<Product, Integer> {
    ...
    @Query("""
                SELECT new backend.cnpm.dto.response.UserProductResponse(
                    p.id,
                    p.productCode,
                    p.productName,
                    ...
                    (SELECT MAX(pd4.salePrice) FROM ProductDetail pd4 WHERE pd4.product.id = p.id),
                    (SELECT pd5.description FROM ProductDetail pd5 WHERE pd5.product.id = p.id ORDER BY pd5.id ASC FETCH FIRST 1 ROWS ONLY),
                    (SELECT CASE WHEN COUNT(pd6) > 0 THEN true ELSE false END FROM ProductDetail pd6 WHERE pd6.product.id = p.id AND pd6.promotion IS NOT NULL AND pd6.promotion.status = true AND pd6.promotion.startDate <= CURRENT_TIMESTAMP AND pd6.promotion.endDate >= CURRENT_TIMESTAMP)
                )
                FROM Product p
    ...
    @Query("SELECT CASE WHEN COUNT(p) > 0 THEN true ELSE false END FROM Product p WHERE p.productName = :productName AND p.id <> :id")
    boolean existsByProductNameAndNotId(@Param("productName") String productName, @Param("id") Integer id);
