...
public interface ProductDetailRepository extends JpaRepository<ProductDetail, Integer> {
    ...
    @Query("""
                SELECT new backend.cnpm.dto.response.ProductDetailResponse(
                    pd.id,
                    new backend.cnpm.dto.response.ProductResponse(
                        p.id,
                        new backend.cnpm.dto.response.BrandResponse(p.brand.id, p.brand.brandName, p.brand.status),
                        new backend.cnpm.dto.response.CategoryResponse(p.category.id, p.category.categoryName, p.category.status),
                        new backend.cnpm.dto.response.MaterialResponse(p.material.id, p.material.materialName, p.material.status),
                        p.productName,
                        p.productCode,
                        p.status
                    ),
                    new backend.cnpm.dto.response.SizeResponse(s.id, s.sizeName, s.status),
                    new backend.cnpm.dto.response.ColorResponse(c.id, c.colorName, c.status),
                    new backend.cnpm.dto.response.PromotionResponse(pr.id, pr.promotionName, pr.promotionPercent, pr.startDate, pr.endDate, pr.description, pr.status),
                    new backend.cnpm.dto.response.CollarResponse(col.id, col.collarName, col.status),
                    new backend.cnpm.dto.response.SleeveResponse(sl.id, sl.sleeveName, sl.status),
                    pd.photo,
                ...
                LEFT JOIN Promotion pr ON pd.promotion.id = pr.id AND pr.status = true AND pr.startDate <= CURRENT_TIMESTAMP AND pr.endDate >= CURRENT_TIMESTAMP
