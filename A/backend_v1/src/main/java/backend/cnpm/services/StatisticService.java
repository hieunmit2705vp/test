...
import backend.cnpm.repositories.StatisticRepository;
    ...
    public List<TopCustomerResponse> getTop5Customers(String startDate, String endDate) {
        try {
            ...
            for (Object[] record : rawData) {
                ...
                try {
                    dto.setId(safeInt(record[0], null));
                    dto.setFullname(safeString(record[1], "Khách hàng không xác định"));
                    dto.setPhone(safeString(record[2], null));
                    dto.setEmail(safeString(record[3], null));
                    dto.setTotalOrders(safeInt(record[4], 0));
                    dto.setTotalSpent(safeBigDecimal(record[5]));
                    result.add(dto);
                } catch (Exception e) {
                    e.printStackTrace();
                }
