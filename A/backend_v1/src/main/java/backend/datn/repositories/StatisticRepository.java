...
public interface StatisticRepository extends JpaRepository<ProductDetail, Integer> {
        ...
        @Query(value = """
                        SELECT a.fullname, count(o.id) as total_orders, sum(o.total_bill - COALESCE(o.shipfee, 0)) as total_spent
                        FROM `order` o
        ...
        // Tổng lợi nhuận = (giá bán thực tế - giá nhập) * số lượng bán, trừ phí ship (nếu có)
        @Query(value = """
